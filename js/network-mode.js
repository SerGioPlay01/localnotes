/**
 * Network Mode Toggle — force offline / online mode
 * Stores preference in localStorage: 'ln_network_mode' = 'offline' | 'online' | 'auto'
 * In offline mode: SW intercepts all fetches from cache, no real network calls.
 * In online mode: SW always tries network first (default behaviour).
 */
(function () {
  'use strict';
  ...
  function buildToggle() {
    ...
    const t = (key, fb) => (window.t ? window.t(key) || fb : fb);
    ...
    wrap.innerHTML = `
      ...
      <button class="ln-net-switch" id="lnNetSwitch" type="button"
              aria-checked="false" role="switch"
              aria-label="${t('networkModeLabel', 'Network mode')}">
  ...
  function updateToggleUI(mode) {
    const btn = document.getElementById('lnNetSwitch');
    if (!btn) return;
    const isOnline = mode === 'online';
    btn.setAttribute('aria-checked', String(isOnline));
    btn.closest('.ln-net-toggle').setAttribute('data-mode', mode);
  }

  /* ── i18n refresh (called after language change) ─────── */
  ...
  function init() {
    ...
    /* re-notify SW once it becomes active */
